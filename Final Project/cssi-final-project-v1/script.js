 ...
 *  This function uses QueryString to find a variabe passed by the previous htmlfile
 */
...
function startOver() {
  ...
  } else if (lang == "fr") { // HARDCODED EXAMPLE TO DEMONSTRATE WINNING SCREEN + LANGUAGE KEYBOARD
    word = "était";
  } else {
...
function displaySettings() {
  ...
  p.noStroke();
  if (lang == "eng") {
    p.text("language: english", 0.7 * width, 0.1 * height);
  } else if (lang == "fr") {
    p.text("language: french", 0.7 * width, 0.1 * height);
  } else if (lang == "esp") {
    p.text("language: spanish", 0.7 * width, 0.1 * height);
  } else if (lang == "rus") {
    p.text("language: russian", 0.7 * width, 0.1 * height);
  } else {
    console.log("error");
  }
  p.text("level: " + level, 0.7 * width, 0.15 * height);
...
function youWin() {
  ...
  centreMsg("Press SPACE to restart", 0.17);
  centreMsg("Correct! The word was " + word, 0.9);
  if (lang !== "eng") {
    centreMsg("In English, this means " + dict[word], 0.97);
  }
  p.noLoop();
